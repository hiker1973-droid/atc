...
start "Al Minhad Tower" cmd /k "%~dp0atc.exe --airfield OMDM --srs-addr %SRS% --eam-password %EAM% --tacview-addr %TACVIEW% --dashboard-port 6001 --log-level %LOG%"
%SystemRoot%\System32\timeout.exe /t 5 /nobreak >nul

:: Al Dhafra Tower
start "Al Dhafra Tower" cmd /k "%~dp0atc.exe --airfield OMAM --srs-addr %SRS% --eam-password %EAM% --tacview-addr %TACVIEW% --dashboard-port 6002 --log-level %LOG%"
%SystemRoot%\System32\timeout.exe /t 5 /nobreak >nul

:: Al Ain Tower
start "Al Ain Tower" cmd /k "%~dp0atc.exe --airfield OMAL --srs-addr %SRS% --eam-password %EAM% --tacview-addr %TACVIEW% --dashboard-port 6003 --log-level %LOG%"
%SystemRoot%\System32\timeout.exe /t 5 /nobreak >nul
