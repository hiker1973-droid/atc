...
:: and the flag defaults to PG's 306.3 -- the card's AI Airboss channel here.
::
...
:: runs at a time.
start "Deckboss (Caucasus)" cmd /c "%~dp0atc.exe --airfield UGSB --srs-addr %SRS% --eam-password %EAM% --tacview-addr %TACVIEW% --deckboss-freq 128.6 --deckboss-voice shimmer --handoff-marshal-freq 306.1 --no-atis --dashboard-port 6005 %MIZ_FLAG% --log-level %LOG%"
