...
const (
	...
	batMarshalSyria  = `%~dp0atc.exe --marshal-only --airfield LCRA --marshal-freq 306.1 --marshal-voice coral --srs-addr %SRS% --tacview-addr %TACVIEW% --eam-password %EAM% --dashboard-port 6004 %MIZ_FLAG% --log-level %LOG%`
	...
	batMarshalCauc   = `%~dp0atc.exe --marshal-only --airfield UGSB --marshal-freq 306.1 --marshal-voice coral --srs-addr %SRS% --tacview-addr %TACVIEW% --eam-password %EAM% --dashboard-port 6004 %MIZ_FLAG% --log-level %LOG%`
	batDeckbossCauc  = `%~dp0atc.exe --airfield UGSB --srs-addr %SRS% --eam-password %EAM% --tacview-addr %TACVIEW% --deckboss-freq 128.6 --deckboss-voice shimmer --handoff-marshal-freq 306.1 --no-atis --dashboard-port 6005 %MIZ_FLAG% --log-level %LOG%`
)
...
func TestRoleMatchesProcCaucasusCarrier(t *testing.T) {
	marshalCauc := proc(1, `C:\SkyeyeATC\atc.exe --marshal-only --airfield UGSB --marshal-freq 306.1 --marshal-voice coral --srs-addr localhost:5008 --tacview-addr localhost:42676 --eam-password hunter2 --dashboard-port 6004 --log-level info`)
	deckCauc := proc(2, `C:\SkyeyeATC\atc.exe --airfield UGSB --srs-addr localhost:5008 --eam-password hunter2 --tacview-addr localhost:42676 --deckboss-freq 128.6 --deckboss-voice shimmer --handoff-marshal-freq 306.1 --no-atis --dashboard-port 6005 --log-level info`)
