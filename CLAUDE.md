...
**Speech rate.** `--tts-speed` defaults to `speedDeckboss` (1.10) as of `adac495` — was a hardcoded 1.05. No `start_*.bat` sets the flag, so this is the live rate for **every tower in all theatres plus Command**. Marshal (1.00, deliberately slow for Case III readback) and ATIS (0.97) use their own constants in `cmd/atc/main.go` and are unaffected. Override per-process with `--tts-speed` if one field needs to differ.
