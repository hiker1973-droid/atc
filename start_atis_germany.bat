...
:: --map germany selects the Cold War Germany ATIS set (8 recovery bases, UHF)
:: and broadcasts each in English then German (atisSecondLangForMap).
start "vSFG-7 ATIS (Germany)" cmd /c "%~dp0atc.exe --atis-only --map germany --srs-addr %SRS% --eam-password %EAM% %MIZ_FLAG% --log-level %LOG%"
