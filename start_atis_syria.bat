@echo off
cd /d C:\SkyeyeATC
if not defined SKYEYE_SRS (
    echo ERROR: SKYEYE_SRS env var not set. Run: setx SKYEYE_SRS ^<host:port^>  then open a new cmd.
    pause
    exit /b 1
)
set SRS=%SKYEYE_SRS%
if not defined SRS_EAM (
    echo ERROR: SRS_EAM env var not set. Run: setx SRS_EAM ^<password^>  then open a new cmd.
    pause
    exit /b 1
)
set EAM=%SRS_EAM%
set LOG=info
set GOMAXPROCS=2
set MIZ_FLAG=
if defined SKYEYE_MIZ set MIZ_FLAG=--miz-path "%SKYEYE_MIZ%"

echo [vSFG-7] Starting Syria ATIS (8 stations, English + Arabic)...
:: H4 (OJHR) broadcasts no ILS/TACAN -- the field has no navaids at all.
start "vSFG-7 ATIS (Syria)" cmd /c "%~dp0atc.exe --atis-only --map syria --srs-addr %SRS% --eam-password %EAM% %MIZ_FLAG% --log-level %LOG%"
echo [vSFG-7] Syria ATIS launched.
